 ...
 */
#define KERNEL_VERSION "12.89, 19 May 2022"
/*
 * 19 May 22   MLA        bug: fixed orphan handling in single turn builds.
 * 17 Mar 22   MLA        Bug: fixed spurious LF at end of centre blocks.
 * 10 Mar 22   MLA        bug: fiddle ARG2 if dumping data.
 * 08 Mar 22   MLA        Increased effective save_name from 16 to 32 chars.
 * 07 Mar 22   MLA        Unflag MOVED on entering input().
 * 27 Feb 22   MLA        Re-jigged handling of log file specification.
 * 24 Feb 22   MLA        Minor op showdata now shows name rather than refnos.
...
void shift_dwarvish (char *, int, int);
...
/*====================================================================*/
void *advrealloc (void *buf, int newlen)
{
   void *ptr;
   if ((ptr = realloc (buf, newlen)) == NULL)
   {
      fprintf (stderr,
...
//#  else /* STYLE >= 10 */
      {
         ...
         int been = 0;
#     ifdef BEENHERE
         been = bitest (what, BEENHERE);
#     endif /* BEENHERE */     
#     ifdef TERSE
         terse = bitest (STATUS, TERSE);
#     endif /* TERSE */     
#     ifdef BRIEF
         brief = bitest (STATUS, BRIEF);
#     endif /* BRIEF */
#     ifdef FULL
         full = bitest (STATUS, FULL);
#     endif /* FULL */
         if (full || !(terse || brief))
            ta = long_desc [what];
         else if (terse)
            ...
            ta = been ? ta = brief_desc [what] : long_desc [what];
      }
   }  
//#  endif /* STYLE */
#endif /* LLOC > FLOC */
...
/*===========================================================*/
void get_input (char *inbuf, int insize)
{
   char *cptr;
#ifndef NO_READLINE
   if (!*prompt_line)
...
#ifdef DWARVEN
      if (value [DWARVEN] || extra_dwarvish)
         shift_dwarvish (arg2_word, WORDSIZE, 1);
#endif /* DWARVEN */
...
#ifdef DWARVEN
   if (value [DWARVEN]) shift_dwarvish (orig, WORDSIZE, -1);
#endif /* DWARVEN */
...
#ifdef DWARVEN
   if (value [DWARVEN]) shift_dwarvish (myword, WORDSIZE, -1);
#endif /* DWARVEN */
   if (*myword == '\0')
   {
      *type = NOISE;
...
got_command:
   ...
   if (value[DWARVEN])
   {
      shift_dwarvish (arg1_word, WORDSIZE, -1);
      shift_dwarvish (arg2_word, WORDSIZE, -1);
   }
#endif
   if (separator [tindex] == ' ')
...
#endif /* CONSOLE */
#ifdef DWARVEN
      if (value [DWARVEN])  shift_dwarvish (reply, 10, -1);
#endif /* DWARVEN */
      rp = reply;
      while (*rp == ' ') rp++;
      if (*rp == '\0' || *rp == '\n') which = 1;
...
try_again:
   ...
   "Can't see any saved games here, but you may know of some elsewhere.\n")
            ...
            get_input (file_name, 16);
#if CONSOLE
            scrchk (NULL);
#endif /* CONSOLE */
#ifdef DWARVEN
            if (value [DWARVEN]) shift_dwarvish (file_name, 16, -1);
#endif /* DWARVEN */
...
#  endif /* DBSTATUS == 3 */
      ...
      else if (*kwrd == 'l')
      {
         if (oc) 
         {
            ...
            if (*opt != SEP)
            {
               getcwd (cp, sizeof (log_path));
               cp += strlen(cp);
               *cp++ = SEP;
            }
            strcpy (log_path + strlen (log_path), opt);
            if (stat (log_path, &stat_buf) == 0 && stat_buf.st_mode & S_IFDIR)
               sprintf (log_path + strlen (log_path), "%c%s.log",
                  SEP, autoname + 1);
         }
         else
            sprintf (log_path, "%s.log", autoname + 1);
...
/*===========================================================*/
void shift_dwarvish (char *aptr, int maxlen, int shift)
{
   char *sptr = aptr;
   while (maxlen-- && *aptr)
   {
      if ((*aptr >= 'a' && *aptr <= 'z') || (*aptr >= 'A' && *aptr <= 'Z'))
         *aptr += shift;
      if (*aptr == 'a' - 1 || *aptr == 'A' - 1)
         *aptr += 26;
      if (*aptr == 'z' + 1 || *aptr == 'Z' + 1)
         *aptr -= 26;
      aptr++;
   }
   if (log_file && shift == -1) fprintf (log_file, "TRANSLATION: %s\n", sptr);
}
/*===========================================================*/
...
/*===========================================================*/
void show_bits (int refno, int size, void *out)
{
   int i;
   fprintf (out, "    ");
   for (i = 0; i < 8 * size * sizeof(short); i++)
      fprintf (out, "%c", bitest (refno, i) ? '1' : '0');
}
/*===========================================================*/
int show_name(int i, void *out)
{
   int len = -1;
#ifdef ENTNAME
   len = 0;
...
void show_data ()
{
   ...
   {
#if STYLE >= 11
      int len = strlen (orig2);
      if (strncmp ("objects", orig2, len) == 0) dtype = 'o';
      else if (strncmp ("places", orig2, len) == 0) dtype = 'l';
      else if (strncmp ("locations", orig2, len) == 0) dtype = 'l';
      else if (strncmp ("variables", orig2, len) == 0) dtype = 'v';
      else if (strncmp ("vars", orig2, len) == 0) dtype = 'v';
      else if (strncmp ("texts", orig2, len) == 0) dtype = 't';
#else
      dtype = *arg2_word;   /* Not completely reliable, but good enough */
#endif /* STYLE */
   }
   if (! dump)
     dump = log_file ? log_file : stderr;
   for (i = FOBJ; i < LTEXT; i++)
   {
      char *type = NULL;
      if (dtype != 'a')
      {
         if (dtype == 'o' && (i < FOBJ || i > LOBJ)) continue;
         if (dtype == 'l' && (i < FLOC || i > LLOC)) continue;
         if (dtype == 'v' && (i < FVAR || i >= LVAR)) continue;
         if (dtype == 't' && (i < FTEXT || i > LTEXT)) continue;
      }
      if (i == FOBJ)
        type = "OBJECTS";
      else if (i == FLOC)
      ...
      fprintf (dump, "%4d ", i);
      if ((j = show_name (i, dump)) !=-1 && i < FVERB || i > LVERB)
      {
            while (j++ < 23)
               fputc (' ', dump);
      }
      if (i >= FTEXT && i <= LTEXT)
      {
         int type = text_info [2 * (i - FTEXT)];
         if (type != 16)
         {
            fprintf (dump, "%5d", *(value + i));
            if (type == RANDOM_TEXT)
               fprintf (dump, " random");
            else if (type == INCREMENTAL_TEXT)
               fprintf (dump, " incremental");
            else if (type == CYCLIC_TEXT)
               fprintf (dump, " cyclic");
            else if (type == ASSIGNED_TEXT)
               fprintf (dump, " assigned");
         }
         else if (type == TIED_TEXT)
         {
            fprintf (dump, "%5d tied to %4d ", value [value [i]], value [i]);
            show_name (value [i], dump);
         }
      }
      else if (i < FVERB || i > LVERB)
         fprintf (dump, "%5d", *(value + i));
      if (i >= FOBJ && i <= LOBJ)
      {
         show_bits (i, OBJSIZE, dump);
         fprintf (dump, " at %4d ", location [i]);
         show_name(location [i], dump);
      }
      else if (i >= FLOC && i <= LLOC )
         show_bits (i, LOCSIZE, dump);
      else if (i >= FVAR && i < FTEXT ) /* Allow for acdc oops! */
      {
         if (*bitword(i) == -1 && value[i] >= 0)
         {
            fprintf (dump, "%s", " => ");
            if (show_name(value [i], dump) == 0)
               show_bits (i, VARSIZE, dump);
         }
         else
           show_bits (i, VARSIZE, dump);
      } 
   ...
   if (dump == log_file)
      PRINTF1 ("Game data dumped into the log file.\n")
 }
/*===========================================================*/
#ifdef UNDO
